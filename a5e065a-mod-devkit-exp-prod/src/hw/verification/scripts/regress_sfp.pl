...
my $scripts=`pwd`;
my $sanitytests = `echo \${ROOTDIR}/verification/tb/tests/sequences/sm_eth_sfp_seq_list.sv`;
my @tests_to_run ;
chomp $scripts;
...
system("gmake -f Makefile.mk cmplib");
system("gmake -f Makefile.mk compile_tb elab $coverage");
...
foreach my $item(@tests_to_run) {
  ...
  while (my $test = <$sh>) {
    ...
    next if($test =~ m/`include "(sm_eth_null_virtual_seq).sv"/);
    next if($test =~ m/`include "(sm_eth_simple_reset_seq).sv"/);
    next if($test =~ m/`include "(sm_eth_axi_master_base_seq).sv"/);
    next if($test =~ m/`include "(sm_eth_basic_seq).sv"/);
    next if($test =~ m/`include "(sm_eth_msgdma_cfg_seq).sv"/);
    next if($test =~ m/`include "(sm_eth_axi_slave_msgdma_host_response_seq).sv"/);
    next if($test =~ m/`include "(sm_eth_basic_data_path_seq).sv"/);
    next if($test =~ m/`include "(sm_eth_sfp_basic_seq).sv"/);

    if($test =~ m/`include "(sm_eth_.*).sv"/)
    {
      print "$1\n";
...
my $sender = 'mrafiqah@ecsmtp.sc.altera.com';
my $receiver = 'shwetank.lal@altera.com alekhya.yerramreddy@altera.com ipsita.das@altera.com muskaan.rafiq.ahmed@altera.com';
#my $cc_receiver = 'Silicom_HW@intel.com';
...
my $msg = MIME::Lite->new(
    ...
    To      => $receiver,
    #Cc      => $cc_receiver,
    Subject => "Regression results for SM ETH ED UVM simulations - Tool: $simulator",
  #  Data    => $msg_body,
