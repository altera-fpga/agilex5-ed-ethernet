...
if [[ -n "${SOLUTION}" ]]; then
	if [[ "$SOLUTION" == "ETH_1P10G" ]]; then
		eth_sed_sw_version="-altera-eth-sed-release-R1.1"
		ETH_SW_VERSION_STRING=${eth_sed_sw_version}
	fi
fi

if [ -n "${ETH_SW_VERSION_STRING}" ]; then
	echo "ETH_SW_VERSION       = $ETH_SW_VERSION_STRING"
	SW_VERSION_STRING=${eth_sed_sw_version}
fi
...
#------------------------------------------------------------------------------------------#
	pushd $WORKSPACE > /dev/null
		...
		if [[ "$MACHINE" == "agilex5_modular" && -n "${SOLUTION}" ]]; then
			sed -i 's/kernel.itb/kernel_sed.itb/' meta-intel-fpga-refdes/conf/machine/agilex5_modular-gsrd.conf
		fi
	popd > /dev/null
